...
import CallHistoryLayout from './user-settings/links-history';
import { AvatarWithBadge } from '@/core/ui/avatar';
import { IoSettingsOutline } from 'react-icons/io5';
import { BsFilter, BsLinkedin } from 'react-icons/bs';
...
const chatWithPatient = [
  {
    id: 1,
    name: 'Isabella Murphin',
    date: 'Today at 9:05 AM',
    message: 'Hi! I’d like to ask if Dr. Emily is available this Friday?',
    photo: '',
    messageClassName: 'bg-[#DDE7FF] text-black',
  },
  {
    id: 2,
    ...
    message:
      'Hi Alicia 👋 Yes, Dr. Emily has a few slots left on Friday. Would you prefer morning or afternoon?',
    photo: '/avatars/alicia.png',
    className: '!bg-blue-100 !text-black',
    messageClassName: 'bg-gray-100',
    divClassName: '',
  },
  {
    id: 1,
...
const SocialMediaCommunication = () => {
  ...
  const [tabActive, setTabActive] = useState('instagram');
  const tabItems = [
    {
      label: (
        <div className="flex items-center gap-1">
          {tabActive == 'instagram' && <RiInstagramFill />}
          <span>Instagram</span>
        </div>
      ),
      value: 'instagram',
    ...
    },
    {
      label: (
        <div className="flex items-center gap-1">
          {tabActive == 'facebook' && <FaSquareFacebook />}
          <span>Facebook</span>
        </div>
      ),
      value: 'facebook',
  ...
  const instagramPersons = persons.filter(
    (person) => person.social_media == 'instagram'
  );
  const facebookPersons = persons.filter(
    (person) => person.social_media == 'facebook'
  );
  const linkedinPersons = persons.filter(
    (person) => person.social_media == 'linkedin'
  );
        ...
        <div className="z-10 flex-1 overflow-y-auto">
          <div className="flex flex-col gap-6 px-4 py-2">
            ...
            {tabActive == 'instagram' && (
              <div>
                <ul className="flex flex-col gap-2">
                  {instagramPersons.map((item, index) => (
                    <li
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex flex-col">
                          ...
                          <span className="text-xs text-gray-500">
                            {item.isActive ? 'Online' : item.time}
                          </span>
                        </div>
                      </div>
                      {item.read_message > 0 && (
            ...
            {tabActive == 'linkedin' && (
              <div>
                <ul className="flex flex-col gap-2">
                  {linkedinPersons.map((item, index) => (
                    <li
                      className={`flex cursor-pointer items-center justify-between rounded-lg px-2 py-2 hover:bg-gray-100 ${selectedUser?.id === item.id && 'rounded-lg bg-[#3872F90D]'} `}
                      key={index}
                      onClick={() => {
            ...
            {tabActive == 'facebook' && (
              <div>
                <ul className="flex flex-col gap-2">
                  {facebookPersons.map((item, index) => (
                    <li
                      className={`flex cursor-pointer items-center justify-between rounded-lg px-2 py-2 hover:bg-gray-100 ${selectedUser?.id === item.id && 'rounded-lg bg-[#3872F90D]'} `}
                      key={index}
                      onClick={() => {
...
export default SocialMediaCommunication;
